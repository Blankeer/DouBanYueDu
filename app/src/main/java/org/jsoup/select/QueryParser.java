...
class QueryParser {
    ...
    static {
        combinators = new String[]{",", SimpleComparison.GREATER_THAN_OPERATION, "+", "~", " "};
        AttributeEvals = new String[]{SimpleComparison.EQUAL_TO_OPERATION, "!=", "^=", "$=", "*=", "~="};
    ...
    private String consumeSubQuery() {
        ...
        while (!this.tq.isEmpty()) {
            if (this.tq.matches("(")) {
                sq.append("(").append(this.tq.chompBalanced(Char.LEFT_PARENTHESIS, Char.RIGHT_PARENTHESIS)).append(")");
    ...
    private void byAttribute() {
        ...
        } else if (cq.matchChomp(SimpleComparison.EQUAL_TO_OPERATION)) {
    ...
    private void cssNthChild(boolean backwards, boolean ofType) {
        ...
        if ("odd".equals(argS)) {
            ...
            if (mAB.group(3) != null) {
                a = Integer.parseInt(mAB.group(1).replaceFirst("^\\+", Table.STRING_DEFAULT_VALUE));
            }
            if (mAB.group(4) != null) {
                b = Integer.parseInt(mAB.group(4).replaceFirst("^\\+", Table.STRING_DEFAULT_VALUE));
            }
        } else if (mB.matches()) {
            a = 0;
            b = Integer.parseInt(mB.group().replaceFirst("^\\+", Table.STRING_DEFAULT_VALUE));
        } else {
    ...
    private void has() {
        this.tq.consume(":has");
        String subQuery = this.tq.chompBalanced(Char.LEFT_PARENTHESIS, Char.RIGHT_PARENTHESIS);
    ...
    private void contains(boolean own) {
        ...
        String searchText = TokenQueue.unescape(this.tq.chompBalanced(Char.LEFT_PARENTHESIS, Char.RIGHT_PARENTHESIS));
    ...
    private void matches(boolean own) {
        ...
        String regex = this.tq.chompBalanced(Char.LEFT_PARENTHESIS, Char.RIGHT_PARENTHESIS);
