            ...
            while (p < len) {
                if (state == 0) {
                    while (p + Base64.CRLF <= len) {
                        value = (((alphabet[input[p] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT] << 18) | (alphabet[input[p + Base64.NO_PADDING] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT] << 12)) | (alphabet[input[p + Base64.NO_WRAP] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT] << 6)) | alphabet[input[p + 3] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT];
                        if (value >= 0) {
                ...
                int d = alphabet[input[p] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT];
                switch (state) {
                    case Base64.DEFAULT /*0*/:
                        if (d < 0) {
                            if (d == SKIP) {
...
*/
        ...
        L_0x022e:
            ...
            goto L_0x0052;
            */
            throw new UnsupportedOperationException("Method not decompiled: com.douban.book.reader.util.Base64.Encoder.process(byte[], int, int, boolean):boolean");
        }
    }

    public static byte[] decode(String str, int flags) throws UnsupportedEncodingException {
        return decode(str.getBytes(HttpRequest.CHARSET_UTF8), flags);
    }
    ...
    public static String encodeToString(byte[] input, int flags) {
        try {
            return new String(encode(input, flags), a.b);
    ...
    public static String encodeToString(byte[] input, int offset, int len, int flags) {
        try {
            return new String(encode(input, offset, len, flags), a.b);
        } catch (UnsupportedEncodingException e) {
            throw new AssertionError(e);
        }
    }
