...
class InstallReferrerUtil {
        ...
        if (installReferrer == null) {
            synchronized (InstallReferrerUtil.class) {
                if (installReferrer == null) {
                    ...
                    if (settings != null) {
                        installReferrer = settings.getString(PREF_KEY_REFERRER, Table.STRING_DEFAULT_VALUE);
                    } else {
                        installReferrer = Table.STRING_DEFAULT_VALUE;
                    }
                }
            }
        }
    ...
    static String getClickReferrer(Context context, String conversionId, String component) {
        ...
        if (referrer == null) {
            ...
            if (settings != null) {
                referrer = settings.getString(conversionId, Table.STRING_DEFAULT_VALUE);
            } else {
                referrer = Table.STRING_DEFAULT_VALUE;
            }
            clickReferrers.put(conversionId, referrer);
        }
