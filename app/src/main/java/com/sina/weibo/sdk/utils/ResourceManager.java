...
public class ResourceManager {
    ...
    private static String getCurrentDpiFolder(Context context) {
        ...
        if (density <= Header.ARRAY_LONG_BYTE) {
            return DRAWABLE_LDPI;
        }
        if (density > Header.ARRAY_LONG_BYTE && density <= Header.ARRAYLIST_PACKED_LONG) {
            return DRAWABLE_MDPI;
        }
        if (density > Header.ARRAYLIST_PACKED_LONG && density <= 240) {
    ...
    public static String readCountryFromAsset(Context context, String assetName) {
        String content = Table.STRING_DEFAULT_VALUE;
        try {
            ...
            dIs.read(buffer);
            content = EncodingUtils.getString(buffer, HttpRequest.CHARSET_UTF8);
            is.close();
            return content;
        } catch (IOException e) {
            e.printStackTrace();
