...
class AnswersRetryFilesSender implements FilesSender {
    ...
    public static AnswersRetryFilesSender build(SessionAnalyticsFilesSender filesSender) {
        return new AnswersRetryFilesSender(filesSender, new RetryManager(new RetryState(new RandomBackoff(new ExponentialBackoff(1000, BACKOFF_POWER), JITTER_PERCENT), new DefaultRetryPolicy(MAX_RETRIES))));
    }
