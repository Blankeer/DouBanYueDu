...
class NativeCrashWriter {
    ...
    private static final class CustomAttributeMessage extends ProtobufMessage {
        ...
        public int getPropertiesSize() {
            return CodedOutputStream.computeBytesSize(1, ByteString.copyFromUtf8(this.key)) + CodedOutputStream.computeBytesSize(PROTOBUF_TAG, ByteString.copyFromUtf8(this.value == null ? Table.STRING_DEFAULT_VALUE : this.value));
        }

        public void writeProperties(CodedOutputStream cos) throws IOException {
            cos.writeBytes(1, ByteString.copyFromUtf8(this.key));
            cos.writeBytes(PROTOBUF_TAG, ByteString.copyFromUtf8(this.value == null ? Table.STRING_DEFAULT_VALUE : this.value));
        }
    }
    ...
    static {
        DEFAULT_SIGNAL = new SignalData(Table.STRING_DEFAULT_VALUE, Table.STRING_DEFAULT_VALUE, 0);
