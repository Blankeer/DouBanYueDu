...
public class QueueFile implements Closeable {
    ...
    private static int readInt(byte[] buffer, int offset) {
        return ((((buffer[offset] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT) << 24) + ((buffer[offset + 1] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT) << HEADER_LENGTH)) + ((buffer[offset + 2] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT) << 8)) + (buffer[offset + 3] & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT);
    }
    ...
    public String toString() {
        ...
        try {
            forEach(new AnonymousClass1(builder));
        } catch (IOException e) {
