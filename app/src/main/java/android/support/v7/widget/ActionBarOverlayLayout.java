...
public class ActionBarOverlayLayout extends ViewGroup implements DecorContentParent, NestedScrollingParent {
    ...
    public void onWindowSystemUiVisibilityChanged(int visible) {
        ...
        if ((visible & WorksIdentity.ID_BIT_FINALIZE) != 0) {
            stable = true;
        } else {
            stable = false;
        }
        ...
        if ((diff & WorksIdentity.ID_BIT_FINALIZE) != 0 && this.mActionBarVisibilityCallback != null) {
            ViewCompat.requestApplyInsets(this);
        }
    }
    ...
    protected boolean fitSystemWindows(Rect insets) {
        pullChildren();
        boolean stable;
        if ((ViewCompat.getWindowSystemUiVisibility(this) & WorksIdentity.ID_BIT_FINALIZE) != 0) {
            stable = true;
        } else {
            stable = false;
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        boolean stable = (ViewCompat.getWindowSystemUiVisibility(this) & WorksIdentity.ID_BIT_FINALIZE) != 0;
        if (stable) {
    ...
    private boolean shouldHideActionBarOnFling(float velocityX, float velocityY) {
        this.mFlingEstimator.fling(0, 0, 0, (int) velocityY, 0, 0, ExploreByTouchHelper.INVALID_ID, AdvancedShareActionProvider.WEIGHT_MAX);
