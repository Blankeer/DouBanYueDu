...
class SuggestionsAdapter extends ResourceCursorAdapter implements OnClickListener {
    ...
    public Cursor runQueryOnBackgroundThread(CharSequence constraint) {
        String query = constraint == null ? Table.STRING_DEFAULT_VALUE : constraint.toString();
    ...
    private Drawable getDrawableFromResourceValue(String drawableId) {
        Drawable drawable;
        if (drawableId == null || drawableId.length() == 0 || Constants.VIA_RESULT_SUCCESS.equals(drawableId)) {
            return null;
        }
        try {
    ...
    Cursor getSearchManagerSuggestions(SearchableInfo searchable, String query, int limit) {
        ...
        Builder uriBuilder = new Builder().scheme(WidgetRequestParam.REQ_PARAM_COMMENT_CONTENT).authority(authority).query(Table.STRING_DEFAULT_VALUE).fragment(Table.STRING_DEFAULT_VALUE);
