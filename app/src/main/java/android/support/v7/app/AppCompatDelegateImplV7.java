...
class AppCompatDelegateImplV7 extends AppCompatDelegateImplBase implements Callback, LayoutInflaterFactory {
    ...
    private final class ActionMenuPresenterCallback implements MenuPresenter.Callback {
        ...
        public boolean onOpenSubMenu(MenuBuilder subMenu) {
            ...
            if (cb != null) {
                cb.onMenuOpened(Header.DOUBLE, subMenu);
            }
            return true;
        }
    ...
    AppCompatDelegateImplV7(Context context, Window window, AppCompatCallback callback) {
        ...
        this.mInvalidatePanelMenuRunnable = new Runnable() {
            public void run() {
                ...
                if ((AppCompatDelegateImplV7.this.mInvalidatePanelMenuFeatures & CodedOutputStream.DEFAULT_BUFFER_SIZE) != 0) {
                    AppCompatDelegateImplV7.this.doInvalidatePanelMenu(Header.DOUBLE);
                }
        ...
        if (!this.mSubDecorInstalled) {
            ...
            if (!isDestroyed()) {
                if (st == null || st.menu == null) {
                    invalidatePanelMenu(Header.DOUBLE);
                }
            }
        }
    }
    ...
    private ViewGroup createSubDecor() {
        ...
        if (a.hasValue(R.styleable.Theme_windowActionBar)) {
            if (a.getBoolean(R.styleable.Theme_windowNoTitle, false)) {
                ...
                requestWindowFeature(Header.DOUBLE);
            }
            if (a.getBoolean(R.styleable.Theme_windowActionBarOverlay, false)) {
                requestWindowFeature(Header.ARRAY_BYTE);
            }
            ...
            if (this.mWindowNoTitle) {
                ...
                if (this.mOverlayActionBar) {
                    this.mDecorContentParent.initFeature(Header.ARRAY_BYTE);
                }
    ...
    public boolean requestWindowFeature(int featureId) {
        ...
        if (this.mWindowNoTitle && featureId == Header.DOUBLE) {
            return false;
        }
        ...
        if (!st.isOpen && !isDestroyed()) {
            ...
            if (cb == null || cb.onMenuOpened(st.featureId, st.menu)) {
                ...
                if (wm != null && preparePanel(st, event)) {
                    ...
                    st.isHandled = false;
                    WindowManager.LayoutParams lp2 = new WindowManager.LayoutParams(width, -2, st.x, st.y, AidTask.WHAT_LOAD_AID_ERR, 8519680, -3);
    ...
    private void reopenMenu(MenuBuilder menu, boolean toggleMenuMode) {
        ...
        if (this.mDecorContentParent.isOverflowMenuShowing() && toggleMenuMode) {
            ...
            if (!isDestroyed()) {
                cb.onPanelClosed(Header.DOUBLE, getPanelState(0, true).menu);
            }
            ...
            if (st.menu != null && !st.refreshMenuContent && cb.onPreparePanel(0, st.createdPanelView, st.menu)) {
                cb.onMenuOpened(Header.DOUBLE, st.menu);
    ...
    private boolean initializePanelMenu(PanelFeatureState st) {
        Context context = this.mContext;
        if ((st.featureId == 0 || st.featureId == Header.DOUBLE) && this.mDecorContentParent != null) {
            TypedValue outValue = new TypedValue();
    ...
    private boolean preparePanel(PanelFeatureState st, KeyEvent event) {
        ...
        if (st.featureId == 0 || st.featureId == Header.DOUBLE) {
    ...
    private void checkCloseActionMenu(MenuBuilder menu) {
        if (!this.mClosingActionMenu) {
            ...
            Window.Callback cb = getWindowCallback();
            if (!(cb == null || isDestroyed())) {
                cb.onPanelClosed(Header.DOUBLE, menu);
            }
    ...
    private void doInvalidatePanelMenu(int featureId) {
        ...
        st.refreshDecorView = true;
        if ((featureId == Header.DOUBLE || featureId == 0) && this.mDecorContentParent != null) {
