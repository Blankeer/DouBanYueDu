...
public abstract class RoundedBitmapDrawable extends Drawable {
    ...
    public void setTargetDensity(int density) {
        if (this.mTargetDensity != density) {
            if (density == 0) {
                density = Header.ARRAYLIST_PACKED_LONG;
            }
    ...
    public int getOpacity() {
        if (this.mGravity != Header.ARRAY_INT || this.mIsCircular) {
            return -3;
        }
        Bitmap bm = this.mBitmap;
        if (bm == null || bm.hasAlpha() || this.mPaint.getAlpha() < SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT || isGreaterThanZero(this.mCornerRadius)) {
            return -3;
        }
        return -1;
    }
    ...
    RoundedBitmapDrawable(Resources res, Bitmap bitmap) {
        this.mTargetDensity = Header.ARRAYLIST_PACKED_LONG;
        this.mGravity = Header.ARRAY_INT;
