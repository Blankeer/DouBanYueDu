...
public class ViewPager extends ViewGroup {
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfoCompat info) {
            ...
            if (ViewPager.this.canScrollHorizontally(ViewPager.SCROLL_STATE_DRAGGING)) {
                info.addAction((int) CodedOutputStream.DEFAULT_BUFFER_SIZE);
            }
        ...
        for (i = SCROLL_STATE_IDLE; i < size; i += SCROLL_STATE_DRAGGING) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int vgrav = lp.gravity & Header.ARRAY_SHORT;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        int action = ev.getAction() & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT;
                ...
                switch (action & SettingsJsonConstants.SETTINGS_IDENTIFIER_MASK_DEFAULT) {
                    ...
                    case SCROLL_STATE_DRAGGING /*1*/:
                        if (this.mIsBeingDragged) {
                            ...
                            velocityTracker.computeCurrentVelocity(AppUri.OPEN_URL, (float) this.mMaximumVelocity);
    ...
    private int determineTargetPage(int currentPage, float pageOffset, int velocity, int deltaX) {
        ...
        if (Math.abs(deltaX) <= this.mFlingDistance || Math.abs(velocity) <= this.mMinimumVelocity) {
            targetPage = (int) ((((float) currentPage) + pageOffset) + (currentPage >= this.mCurItem ? CharUtils.FULL_WIDTH_CHAR_OFFSET_ADJUSTMENT_RATIO : 0.6f));
        } else {
